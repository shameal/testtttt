...
private:
	...
	/*****************************************************************/
        ...
        ros::Subscriber ultra_subscriber;
        ros::Subscriber bodyvelocity_subscriber;
...
public:
    ...
    geometry_msgs::TwistStamped ideal_motion;
    geometry_msgs::TwistStamped initial_motion;
    geometry_msgs::Vector3Stamped bodyvelocity;

    int mission_start_time;
    bool missionflag;
    bool jiang_missionflag;
    int jiang_mission_start_time;
    int updateflight=0;  
    float roll;
	float pitch;
	float yaw;
    float ideal_yaw=0;     
    bool completeflag=1;
    int time_stamp_angular=0;
    double v_max=0.3,v_max_yaw=20,v_max_z=0.1;
    double err_p_x,err_p_y,err_p_z,err_p_yaw;
	double err_i_x,err_i_y,err_i_z,err_eli_x=v_max*0.5,err_eli_y=v_max*0.5,err_eli_z=v_max*0.5;
	double err_d_x,err_d_y,err_d_z;
	double ctrl_out_x,ctrl_out_y, ctrl_out_z,ctrl_out_yaw;
	double k_p=1,k_i=0,k_d=0,k_p_z=1;
...
private:
	/*********************************************************************************/\
	void pid_subscriber_callback(geometry_msgs::TwistStamped motion)
    {
               //Control value based on guidance feedback
               this->err_p_x=this->ideal_motion.twist.angular.x-motion.twist.angular.x;
               this->err_p_y=this->ideal_motion.twist.angular.y-motion.twist.angular.y;
               this->err_p_z=this->ideal_motion.twist.angular.z-this->motion.twist.angular.z;
               this->err_p_yaw=this->ideal_yaw-this->yaw;
   

               this->err_d_x=motion.twist.linear.x;
               this->err_d_y=motion.twist.linear.y;  
               this->err_d_z=-1.0*motion.twist.linear.z;

               this->err_i_x+=this->err_p_x*((motion.header.stamp.nsec-this->motion.header.stamp.nsec)*pow(10,-9));
               this->err_i_x=((this->err_i_x)<(-1.0*err_eli_x)? (-1.0*err_eli_x): ((this->err_i_x)>(err_eli_x)?(err_eli_x):(this->err_eli_x)));
               this->err_i_y+=this->err_p_y*((motion.header.stamp.nsec-this->motion.header.stamp.nsec)*pow(10,-9));
               this->err_i_y=((this->err_i_y)<(-1.0*err_eli_y)? (-1.0*err_eli_y): ((this->err_i_y)>(err_eli_y)?(err_eli_y):(this->err_eli_y)));
               this->err_i_z+=this->err_p_z*((motion.header.stamp.nsec-this->motion.header.stamp.nsec)*pow(10,-9));
               this->err_i_z=((this->err_i_z)<(-1.0*err_eli_z)? (-1.0*err_eli_z): ((this->err_i_z)>(err_eli_z)?(err_eli_z):(this->err_eli_z)));
                  
               
               this->ctrl_out_x=this->k_p*this->err_p_x+this->k_d*this->err_d_x+this->k_i*this->err_i_x;
               this->ctrl_out_x=((this->ctrl_out_x)<(-1.0*v_max)? (-1.0*v_max): ((this->ctrl_out_x)>(v_max)?(v_max):(this->ctrl_out_x)));
               this->ctrl_out_y=this->k_p*this->err_p_y+this->k_d*this->err_d_y+this->k_i*this->err_i_y;
               this->ctrl_out_y=((this->ctrl_out_y)<(-1.0*v_max)? (-1.0*v_max): ((this->ctrl_out_y)>(v_max)?(v_max):(this->ctrl_out_y)));
               this->ctrl_out_z=this->k_p_z*this->err_p_z+this->k_d*this->err_d_z+this->k_i*this->err_i_z;
               this->ctrl_out_z=((this->ctrl_out_z)<(-1.0*v_max_z)? (-1.0*v_max_z): ((this->ctrl_out_z)>(v_max_z)?(v_max_z):(this->ctrl_out_z)));
               this->ctrl_out_yaw=this->k_p*this->err_p_yaw;
               this->ctrl_out_yaw=((this->ctrl_out_yaw)<(-1.0*v_max_yaw)? (-1.0*v_max_yaw): ((this->ctrl_out_yaw)>(v_max_yaw)?(v_max_yaw):(this->ctrl_out_yaw)));

                switch(this->flightmode)
           		{
           		case 0:     //for debug purpose
		       std::cout<<"CALLBACK SUCESSFUL! "<<std::endl;
		       break;

		       case 1:
               this->attitude_control(0x48, this->ctrl_out_x, this->ctrl_out_y, this->ctrl_out_z, this->ctrl_out_yaw);
		       std::cout<<"X Y  Yaw Control: X and Y and Z and Yaw are: "<<this->ctrl_out_x<<" and "<<
		       this->ctrl_out_y<<' '<<this->ctrl_out_z<<' '<<this->ctrl_out_yaw<<std::endl;
		      if(err_p_x<0.01&&err_p_x>-0.01&&err_p_y<0.01&&err_p_y>-0.01&&err_p_z<0.02&&err_p_z>-0.02&&err_p_yaw>-0.5&&err_p_yaw<0.5)
		        {
    ...
    void yaw_subscriber_callback(std_msgs::Float32 yaw)
        {
                this->yaw=yaw.data;
        }

        
    void bodyvelocity_subscriber_callback(geometry_msgs::Vector3Stamped bodyvelocity)
	{
                 this->bodyvelocity=bodyvelocity;
	}
	void ultra_subscriber_callback(sensor_msgs::LaserScan height)
     {
                this->motion.twist.angular.z=height.ranges[0];
     }
	/*********************************************************************************/
...
public:
		...
		waypoint_navigation_action_client(nh, "dji_sdk/waypoint_navigation_action", true)
	{
		...
		/*******************************************************************************************/
                bodyvelocity_subscriber=nh.subscribe<geometry_msgs::Vector3Stamped>("/guidance/velocity",10,&DJIDrone::bodyvelocity_subscriber_callback, this);
                pid_subscriber=nh.subscribe<geometry_msgs::TwistStamped>("/guidance/motion",10,
&DJIDrone::pid_subscriber_callback, this);
                yaw_subscriber=nh.subscribe<std_msgs::Float32>("dji_sdk/yaw",10,
&DJIDrone::yaw_subscriber_callback, this);
                ultra_subscriber=nh.subscribe<sensor_msgs::LaserScan>("/guidance/ultrasonic",10,
&DJIDrone::ultra_subscriber_callback, this);
		/*******************************************************************************************/
