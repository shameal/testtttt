...
void depth_image_callback(const sensor_msgs::ImageConstPtr& depth_img)
{
	...
	/*char save[30];
        sprintf(save, "/home/ubuntu/Pictures/depth%d.bmp", i/5);

   cv::imwrite(save,depth8);
	}*/
    cv::waitKey(1);
}
...
/* motion */
void position_callback(const geometry_msgs::TwistStamped& g_mo)
{
    printf("frame_id: %s stamp: %d\n", g_mo.header.frame_id.c_str(), g_mo.header.stamp.nsec);
        printf("global velocity: [%f %f %f]\n", g_mo.twist.linear.x, g_mo.twist.linear.y, g_mo.twist.linear.z);
        printf("global position: [%f %f %f]\n", g_mo.twist.angular.x, g_mo.twist.angular.y, g_mo.twist.angular.z);
}
...
int main(int argc, char** argv)
{
    ...
    depth_image_sub       = my_node.subscribe("/guidance/depth_image", 1, depth_image_callback);
    ...
    position_sub = my_node.subscribe("/guidance/motion", 1, position_callback);
