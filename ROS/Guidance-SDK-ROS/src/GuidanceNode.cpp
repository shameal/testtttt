...
#include <geometry_msgs/TwistStamped.h> //Motion
...
int my_callback(int data_type, int data_len, char *content)
{
    ...
    if ( e_motion == data_type && NULL != content )
    {
        ...
        if (verbosity > 1) {
            ...
            printf( "gvx:%f gvy:%f gvz:%f\n", mo->velocity_in_global_x,  mo->velocity_in_global_y, mo->velocity_in_global_z);
	        printf( "gpx:%f gpy:%f gpz:%f\n", mo->position_in_global_x,  mo->position_in_global_y, mo->position_in_global_z );
     }
	//publish

	geometry_msgs::TwistStamped g_mo;
    g_mo.header.frame_id = "guidance";
    g_mo.header.stamp    = ros::Time::now();
	g_mo.twist.linear.x =  mo->velocity_in_global_x;
	g_mo.twist.linear.y =  mo->velocity_in_global_y;
	g_mo.twist.linear.z =  mo->velocity_in_global_z;
	g_mo.twist.angular.x =  mo->position_in_global_x;
	g_mo.twist.angular.y =  mo->position_in_global_y;
	g_mo.twist.angular.z =  mo->position_in_global_z;
    ...
    if ( e_ultrasonic == data_type && NULL != content )
    {
        ...
        if (verbosity > 1) {
           ...
           /* for ( int d = 0; d < CAMERA_PAIR_NUM; ++d )
            {*/
                printf( "ultrasonic distance: %f, reliability: %d\n", ultrasonic->ultrasonic[5] * 0.001f, (int)ultrasonic->reliability[5] );
           // }
        }
...
int main(int argc, char** argv)
{
    ...
    motion_pub                          = my_node.advertise<geometry_msgs::TwistStamped>( "/guidance/motion",1);
