...
void MPU6050_getMotion6(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz) {
	...
	if(isMPU6050_is_DRY)
	{
		...
		*gx  =MPU6050_FIFO[3][10] - 0;//GyroSavedCaliData.GyroXOffset;
		*gy = MPU6050_FIFO[4][10] - 0;//GyroSavedCaliData.GyroYOffset;
		*gz = MPU6050_FIFO[5][10] - 0;//GyroSavedCaliData.GyroZOffset;
	} 
	else
	{       //读取上一次的值
		...
		*gx = MPU6050_FIFO[3][10]-0;//GyroSavedCaliData.GyroXOffset;//=MPU6050_FIFO[3][10];
		*gy = MPU6050_FIFO[4][10]-0;//GyroSavedCaliData.GyroYOffset;//=MPU6050_FIFO[4][10];
		*gz = MPU6050_FIFO[5][10]-0;//GyroSavedCaliData.GyroZOffset;//=MPU6050_FIFO[5][10];
	}
}
